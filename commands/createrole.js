exports.run = async (client, message, args) => {
  ...
  if (!message.guild.me.hasPermission("MANAGE_ROLES_OR_PERMISSIONS")) {
    return msg.edit(
      "Sorry, I don't have permissions to **Manage Roles** in this guild, please check my permissions and try again."
    );
  } else {
    ...
    msg.edit(
      `Created the role \`${args[0]}\` with color \`${
        args[1]
      }\` and a hoist of \`${args[2]}\``
    );
  }
};
