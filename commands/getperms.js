...
exports.run = (client, message, args) => {
  const resolvedUser =
    args[0] !== undefined
      ? message.guild.members.get(args[0].match(/[0-9]/g).join(""))
      : null;
  const botuser = resolvedUser
    ? message.guild.members.get(resolvedUser.id)
    : message.member;
  ...
  embed.addField(
    "Server Permissions",
    `${checkUserPermission(message.guild, botuser).join(", ")}`,
    true
  );
  embed.addField(
    "Channel Permissions",
    `${message.channel
      .rolePermissions(botuser.roles.first())
      .toArray()
      .join(", ")}`,
    true
  );
