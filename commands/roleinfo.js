...
exports.run = async (client, message, args) => {
  ...
  const roleM = message.guild.roles.find(
  ...
  try {
    ...
    if (checkRolePermission(message.guild, roleM).length > 0) {
      embed.addField(
        "Key Permissions",
        `${checkRolePermission(message.guild, roleM).join(", ")}`,
        true
      );
    }
    embed.addField(
      "Created At",
      `${roleM.createdAt.toLocaleString("en-US")}`,
      true
    );
    embed.setFooter(
      `${client.user.username} | Role ID: ${roleM.id}`,
